...
class PurePursuit(Node):
    ...
    def _find_lookahead(self, ld: float) -> tuple[float, float] | None:
        """
        Walk forward along the path from path_idx, find the first point
        that lies on the circle of radius ld centred on the robot.
        Returns (x, y) of the intersection, or None.
        """
        robot = (self.x, self.y)

        # Advance path_idx past points the robot has already driven through
        while self.path_idx < len(self.path) - 1:
            if math.hypot(self.path[self.path_idx][0] - robot[0],
                          self.path[self.path_idx][1] - robot[1]) > ld * 0.25:
                break
            self.path_idx += 1

        # Search segments ahead for circle intersection
        for i in range(self.path_idx, len(self.path) - 1):
            hit = self._circle_segment_intersect(robot, ld, self.path[i], self.path[i + 1])
            if hit is not None:
                return hit

        # Fallback: furthest path point within 2× lookahead
        for pt in reversed(self.path):
            if math.hypot(pt[0] - robot[0], pt[1] - robot[1]) <= ld * 2.0:
                return pt
